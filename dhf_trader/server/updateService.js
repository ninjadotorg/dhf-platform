...
async function updateOrdersNotIn (orders, gateway, project) {
  ...
  const ordersNotIn = await OrderDB.find({
    status: { $nin: ['FILLED', 'CANCELED'] },
    exchangeOrderID: { $nin: orderIds }
  })
  ...
  await Promise.map(
    ordersNotIn,
    async o => {
      const detail = await getOrderDetail(gateway, {
...
async function updateOpenOrders (openOrders, gateway) {
  ...
  await Promise.map(
    openOrders,
    o => updateOrder(o.orderId, o.status, o.executedQty),
    { concurrency: 10 }
  )
}
...
async function updateOrders (project) {
  try {
    ...
    await Promise.each(symbols, async symbol => {
      console.log(
        'finding open orders of project',
        project,
        'with symbol',
        symbol
      )
      ...
      await Promise.all([
        updateOpenOrders(openOrders, gateway),
        updateOrdersNotIn(openOrders, gateway, project)
      ])
    })
  } catch (e) {
    console.log(e)
