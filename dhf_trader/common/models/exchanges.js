...
DBModel.getAvailableAccounts = async function(exchange){
    let results = await this.find({name: exchange, isLock: false}).select({"key": 0, "secret": 0});
    return results
}

DBModel.getLockAccounts = async function(exchange){
    let results = await this.find({name: exchange, isLock: true}).select({"key": 0, "secret": 0});
    return results
}
